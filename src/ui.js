...
function notifToggleHTML(key, emoji, label, desc) {
  var n = D.notifications || {morning:false,evening:false,morningTime:'08:00',eveningTime:'20:00',craving:false,journal:false,breathe:false,cravingTime:'14:00',journalTime:'12:00',breatheTime:'10:00',buddyCheckin:false};
  var enabled = n[key] || false;
  return '<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;margin-top:4px"><div style="display:flex;align-items:center;gap:8px"><span style="font-size:18px">'+emoji+'</span><div><div style="font-size:13px;font-weight:600">'+label+'</div><div style="font-size:11px;color:var(--muted)">'+desc+'</div></div></div><label style="position:relative;display:inline-block;width:44px;height:24px;cursor:pointer"><input type="checkbox" '+(enabled?'checked':'')+' onchange="D.notifications.'+key+'=this.checked;if(this.checked)requestNotif();saveDataSilent();var sw=this.nextSibling;sw.style.background=this.checked?\'var(--primary)\':\'var(--border)\';sw.firstChild.style.left=this.checked?\'23px\':\'3px\';if(this.checked){var p=notifPermissionStatus();if(p===\'denied\')showToast(\'Notifications blocked — allow them in browser site settings.\',\'warning\');else if(!("Notification" in window))showToast(\'Notifications not supported by this browser.\',\'warning\');}" style="opacity:0;width:0;height:0"><span style="position:absolute;inset:0;background:'+(enabled?'var(--primary)':'var(--border)')+';border-radius:12px;transition:.2s"><span style="position:absolute;top:3px;left:'+(enabled?'23':'3')+'px;width:18px;height:18px;border-radius:9px;background:#fff;transition:.2s;box-shadow:0 1px 3px rgba(0,0,0,.15)"></span></span></label></div>';
}
function pushToggleRowHTML() {
  var n = D.notifications || {};
  var enabled = n.push !== false;
  var perm = notifPermissionStatus();
  var permText = perm === 'granted' ? '\u2705 Browser notifications allowed' : perm === 'denied' ? '\u274C Notifications blocked by browser \u2014 allow in site settings (padlock icon)' : perm === 'unsupported' ? '\u26A0\uFE0F Notifications not supported' : '\u23F3 Your browser will ask when a notification is due';
  var toastOn = perm === 'granted' ? 'showToast(\'Push notifications enabled.\')' : perm === 'denied' ? 'showToast(\'Notifications blocked \u2014 allow them in browser site settings (padlock icon).\',\'warning\')' : perm === 'unsupported' ? 'showToast(\'Notifications not supported by this browser.\',\'warning\')' : 'showToast(\'Push toggle on \u2014 your browser will ask for permission now.\')';
  return '<div style="margin-bottom:4px;border-bottom:1px solid var(--border);padding-bottom:6px"><div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0"><div style="display:flex;align-items:center;gap:8px"><span style="font-size:18px">&#128276;</span><div><div style="font-size:13px;font-weight:600">Push Notifications</div><div style="font-size:11px;color:var(--muted)">Receive alerts on this device</div></div></div><label style="position:relative;display:inline-block;width:44px;height:24px;cursor:pointer"><input type="checkbox" '+(enabled?'checked':'')+' onchange="D.notifications.push=this.checked;if(this.checked)requestNotif();else unsubscribePush();saveDataSilent();var sw=this.nextSibling;sw.style.background=this.checked?\'var(--primary)\':\'var(--border)\';sw.firstChild.style.left=this.checked?\'23px\':\'3px\';'+(enabled?'showToast(\'Push notifications turned off.\')':toastOn)+'" style="opacity:0;width:0;height:0"><span style="position:absolute;inset:0;background:'+(enabled?'var(--primary)':'var(--border)')+';border-radius:12px;transition:.2s"><span style="position:absolute;top:3px;left:'+(enabled?'23':'3')+'px;width:18px;height:18px;border-radius:9px;background:#fff;transition:.2s;box-shadow:0 1px 3px rgba(0,0,0,.15)"></span></span></label></div><div style="font-size:10px;color:var(--muted);padding-left:28px;margin-top:-2px">'+permText+'</div></div>';
}
function syncNativeReminders() {
  if (typeof Capacitor === 'undefined' || !Capacitor.isNativePlatform || !Capacitor.isNativePlatform()) return;
  if (!Capacitor.Plugins || !Capacitor.Plugins.LocalNotifications) return;
  var all = [];
  var n = D.notifications || {};
  var j = D.journal || [];
  var h = n.morning ? [n.morningTime || '08:00'] : [];
  var f = n.evening ? [n.eveningTime || '20:00'] : [];
  var c = n.craving ? [n.cravingTime || '14:00'] : [];
  var r = n.journal ? [n.journalTime || '12:00'] : [];
  var b = n.breathe ? [n.breatheTime || '10:00'] : [];
  var max = 64;
  pushNativeNotifs(all, h, '08:00', 'Re.Claim Morning', 'Day ' + (D.streak || 0) + ' forward \u2014 log your mood and set your intention.', 201, 'checkin');
  pushNativeNotifs(all, f, '20:00', 'Re.Claim Evening', 'Day ' + (D.streak || 0) + ' made \u2014 journal what happened today.', 202, 'journal');
  pushNativeNotifs(all, c, '14:00', 'Craving Check-In', 'Pause and check in with yourself \u2014 the urge is not you.', 203, 'craving');
  pushNativeNotifs(all, r, '12:00', 'Journal Prompt', 'Write one line about today \u2014 that counts.', 204, 'journal');
  pushNativeNotifs(all, b, '10:00', 'Breathe', 'Step outside the spiral \u2014 2 minutes of breathing.', 205, 'breathe');
  if (all.length > max) all = all.slice(0, max);
  Capacitor.Plugins.LocalNotifications.cancel({ notifications: all.map(function(x) { return { id: x.id }; }) }).catch(function() {});
  if (all.length) {
    Capacitor.Plugins.LocalNotifications.schedule({ notifications: all }).then(function() {
      Capacitor.Plugins.LocalNotifications.requestPermissions().catch(function() {});
    }).catch(function() {});
  }
}
function pushNativeNotifs(list, times, dflt, title, body, baseId, tag) {
  var seen = {};
  for (var i = 0; i < times.length && list.length < 64; i++) {
    var t = times[i] || dflt;
    var hm = t.split(':');
    if (hm.length !== 2) continue;
    var hour = parseInt(hm[0], 10), minute = parseInt(hm[1], 10);
    ...
    list.push({
      ...
      title: title,
      body: body,
      schedule: { on: { hour: hour, minute: minute }, allowWhileIdle: true, repeats: true },
      sound: null,
  ...
  if (typeof Capacitor !== 'undefined' && Capacitor.isNativePlatform && Capacitor.isNativePlatform()) return 'granted';
...
// ====== RENDER & NAV ======
var MORE_SUB_PAGES = ['journal','calendar','reports','buddy','coping','assessment','profile','reminders','meetings','timecapsule','achievements','insights','seer'];
...
function render() {
  ...
  function doRender() {
    ...
    var rh = document.getElementById('journey-header');
    if (rh) {
